...
test_launcher_window_reopen_behavior() {
    ...
    local no_pidfd_bin="$TMP_DIR/launcher-window-reopen-no-pidfd-bin"
    ...
    rm -rf "$no_pidfd_tmp"
    mkdir -p "$no_pidfd_bin"
    cat > "$no_pidfd_bin/python3" <<'SH'
#!/usr/bin/env bash
set -Eeuo pipefail
...
def unavailable_pidfd(*_args, **_kwargs):
    raise OSError(errno.ENOSYS, "pidfd unavailable in controlled test environment")
...
exec "$REAL_PYTHON" "$@"
SH
    chmod +x "$no_pidfd_bin/python3"
    set +e
    REAL_PYTHON="$(PATH="$HOST_TOOL_PATH" type -P python3)" \
        PATH="$no_pidfd_bin:$PATH" TMPDIR="$no_pidfd_tmp" \
        bash "$REPO_DIR/tests/launcher_window_reopen_behavior.sh" \
        > "$no_pidfd_log" 2>&1
    status=$?
    set -e
    ...
    if [ "$status" -ne 1 ] \
        ...
        fail "Window-reopen mutation run misclassified a pidfd probe setup failure (status $status)"
    fi
    [ ! -e "$no_pidfd_tmp" ] \
